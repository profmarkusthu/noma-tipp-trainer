...
export class GameEngine {
    constructor(lesson, config = DEFAULT_GAME_CONFIG) {
        ...
        Object.defineProperty(this, "difficultySystem", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        this.spawnSystem = new SpawnSystem(lesson);
    ...
    update(deltaTime) {
        ...
        this.spawnSystem.spawnRate = baseSpawnRate + speedRatio * (maxDynamicSpawnRate - baseSpawnRate);
        // Spawn new blocks
        ...
        for (const block of this.state.blocks) {
            if (!block.isDestroying) {
                const playerHitsBlock = CollisionSystem.checkAABB(this.state.player, block);
                if (playerHitsBlock) {
        ...
        this.state.blocks = this.state.blocks.filter((block) => {
            ...
            if (block.x + block.width <= 0)
                return false; // Remove off-screen
            if (block.isDestroying && block.destroyAnimationFrame > 10)
                return false; // Remove after animation
            return true;
        });
    ...
    handleKeyPress(key) {
        ...
        if (isCorrect && this.state.targetLetter) {
            ...
            if (this.state.currentWordProgress === this.state.targetLetter) {
                ...
                const blockIndex = this.state.blocks.findIndex((b) => !b.isDestroying && b.letter === this.state.targetLetter);
                if (blockIndex !== -1) {
    ...
    reset(lesson, config = DEFAULT_GAME_CONFIG) {
        this.currentLesson = lesson;
